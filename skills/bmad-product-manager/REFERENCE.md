...
### Language-Aware Authoring

- Resolve output language from project settings before writing requirements.
- Keep labels/headers consistent with template structure, but write requirement prose and field values in the output language.
- Do not mix English modal verbs with non-English requirement text in the same sentence.
- English acronyms/abbreviations are allowed inside non-English text (for example, API, SLA, KPI, OAuth, WCAG).
- Use a language-appropriate normative verb (for example, English `shall`; Russian `должен/должна/должно`).
- Keep `Given`, `when`, `then` in English as notation tokens and format as `**Given**`, `**when**`, `**then**`.
- Localize values for fields like `Compliance`, `Load Profile`, and `Measurement Method`.
...
```
FR-XXX: [Priority] - [System actor in document language] [normative verb in document language] [behavior] [condition]
Acceptance Criteria:
- [Trigger condition]
- [Expected behavior]
- [Error handling]
...
```
FR-XXX: [Priority] - [System actor in document language] [normative verb in document language] store/retrieve/update [data] with [constraints]
Acceptance Criteria:
...
**Integration Pattern:**
```
FR-XXX: [Priority] - [System actor in document language] [normative verb in document language] integrate with [external system] to [purpose]
Acceptance Criteria:
- [Integration method]
...
**Performance Pattern:**
```
NFR-XXX: [Priority] - [Operation in document language] [normative verb in document language] complete within [time] for [percentile] of requests
Example (English): API response shall complete within 200ms for 95th percentile under normal load
Example (Russian): Ответ API должен завершаться за 200 мс для 95-го перцентиля при нормальной нагрузке
```

**Scalability Pattern:**
```
NFR-XXX: [Priority] - [System actor in document language] [normative verb in document language] support [quantity] [resource] with [degradation] degradation
Example (English): System shall support 10,000 concurrent users with <5% performance degradation
Example (Russian): Система должна поддерживать 10 000 одновременных пользователей с деградацией производительности <5%
```

**Security Pattern:**
```
NFR-XXX: [Priority] - [Component in document language] [normative verb in document language] implement [security control] per [standard]
Example (English): API shall implement OAuth 2.0 authentication per RFC 6749
Example (Russian): API должен реализовать аутентификацию OAuth 2.0 согласно RFC 6749
```

**Reliability Pattern:**
```
NFR-XXX: [Priority] - [System actor in document language] [normative verb in document language] maintain [uptime]% availability excluding planned maintenance
Example (English): System shall maintain 99.9% availability excluding scheduled maintenance windows
Example (Russian): Система должна обеспечивать доступность 99.9% за исключением плановых работ
```

**Usability Pattern:**
```
NFR-XXX: [Priority] - [Interface in document language] [normative verb in document language] achieve [metric] compliance/score
Example (English): Application shall achieve WCAG 2.1 AA compliance for all user-facing features
Example (Russian): Приложение должно соответствовать WCAG 2.1 AA для всех пользовательских функций
```

**Maintainability Pattern:**
```
NFR-XXX: [Priority] - [Codebase/component in document language] [normative verb in document language] maintain [metric] above [threshold]
Example (English): Codebase shall maintain test coverage above 80% for critical business logic
Example (Russian): Кодовая база должна поддерживать покрытие тестами выше 80% для критичной бизнес-логики
```
...
```
[Role statement in document language]
[Need statement in document language]
[Benefit statement in document language]
...
1. **The Solution Specification**
   - **Bad:** "Use PostgreSQL database with connection pooling"
   - **Good:** "Persist user data with sub-100ms read latency"

2. **The Vague Requirement**
   - **Bad:** "The product is user-friendly"
   - **Good:** "90% of users complete the core workflow without help docs"
...
5. **The Implementation Constraint**
   ...
   - **Good:** "UI maintains state across page navigation"
